        ...
        if augment == True:
            for r in range(0,3):
                ...
                if centr == True:
                    ...
                    yss.append(np.rot90(y, r))
                    yss.append(np.fliplr(np.rot90(y, r)))
...
def batchload(WORKDIR, train_test_ratio, num_classes, batch_size=8, edgelength=32, centr=True):
    ...
    X_batches, y_batches = load_data_generate_batches(xtr,ytr,num_classes, batch_size,edgelength,True,centr)
    xte, yte = generate_test(xte,yte,edgelength,num_classes,centr)
